...
def funcoes_de_ativacao_derivada(type,number):
    if(type == 1):
        return mp.exp(-number)/(1.0+mp.exp(-number))*(1.0+mp.exp(-number))
    elif(type == 2):
...
def calcular_pesos(entrada, nets, tamanho_camada_x, tamanho_camada_y, pesos_camada):
    for i in range(tamanho_camada_x):
        somatoria_net = 0
        for j in range(tamanho_camada_y):
            somatoria_net = somatoria_net + (entrada[j] * round(pesos_camada[i][j],2))
            #print(somatoria_net)
...
def calcular_propagacao(nets, propagacao, tamanho, modo_execucao):
    for i in range(tamanho):
        propagacao[i] = round(funcoes_de_ativacao(modo_execucao, nets[i]),2)
...
def calcular_erros_da_saida(erros_saida, saidas, entrada, tamanho_camada_saida, nets, modo_execucao):
    ...
    if(modo_execucao == 1):
        ...
        valores_desejados[entrada-1] = 1
    else:
        for i in range(tamanho_camada_saida):
            valores_desejados[i] = -1
        valores_desejados[entrada-1] = 1
    for j in range(tamanho_camada_saida):
        erros_saida[j] = (valores_desejados[j]-saidas[j]) * round(funcoes_de_ativacao_derivada(modo_execucao, nets[j]), 2)
...
def calcular_erros_da_oculta(erros_entrada, tamanho_camada_oculta, erros_saida, tamanho_camada_saida,  nets, pesos_saida, modo_execucao):
    ...
    for i in range(tamanho_camada_oculta):
        somatorio = 0.0
        for j in range(tamanho_camada_saida):
            somatorio = somatorio + (erros_saida[j] * pesos_saida[j][i])
...
def verificar(matriz_de_confusao, classe, tamanho_camada_entrada, saida, tamanho_camada_saida, modo_de_execucao):
    ...
    matriz_de_confusao[classe-1][maior] += 1 
